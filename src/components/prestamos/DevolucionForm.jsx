    ...
    try {
      ...
      for (const detalle of prestamo.prestamos_detalle) {
        const { error: detalleError } = await supabase
          ...
          .update({
            estado_devolucion: estados[detalle.id],
        ...
        // 2. Devolver stock de cada herramienta
        ...
        const { error: updateError } = await supabase
          .from('herramientas')
          .update({ stock: herramienta.stock + detalle.cantidad })
          .eq('id', detalle.herramienta_id);
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t">
            ...
            >
              {loading ? <LoadingSpinner /> : 'Confirmar Devolución'}
            </button>
          </div>
        </form>
      </div>
